...
class LeSageComparacion(Scene):
    """
    LeSage v1.2.1 - Planeta con Perlin Noise heatmap

    Planeta con patrón térmico tipo NASA/AIRS usando Perlin noise.
    Zonas irregulares de temperatura que evolucionan hacia el rojo.

    - Inicio: planeta con patrón frío (púrpura/azul dominante)
    - Lluvia empuja todos los colores hacia el rojo
    - Final: esfera roja incandescente
    """
    ...
    def construct(self):
        ...
        # Crear grid de puntos dentro del círculo
        espaciado = 0.08
        for x in np.arange(-radio_visual, radio_visual + espaciado, espaciado):
            for y in np.arange(-radio_visual, radio_visual + espaciado, espaciado):
                ...
                if dist <= radio_visual * 0.95:
                    ...
                    color = self.noise_a_color(noise_val * 0.4)  # Empezar frío

                    punto = Dot(
                        point=np.array([x + CENTRO[0], y + CENTRO[1], 0]),
                        radius=espaciado * 0.6,
                        color=color,
                        fill_opacity=0.9
                    )
    ...
    def calentamiento_con_contador(self, duracion=25, centro=ORIGIN, label=None, barra_fondo=None):
        ...
        for i in range(num_updates):
            ...
            # ACTUALIZAR COLORES PERLIN NOISE
            # Cada punto se "calienta" - su valor se desplaza hacia el rojo
            for dato in self.datos_puntos:
                # Valor base del noise + offset por progreso
                # Al inicio: noise_base * 0.4 (frío)
                # Al final: noise_base * 0.3 + 0.7 (casi todo rojo)
                valor_actual = dato['noise_base'] * (0.4 - progreso * 0.3) + progreso * 0.85
                valor_actual = max(0, min(1, valor_actual))

                nuevo_color = self.noise_a_color(valor_actual)
                dato['punto'].set_fill(nuevo_color, opacity=0.9)

            # Borde del planeta cambia de color también
            color_borde = self.noise_a_color(progreso * 0.9)
            self.borde_planeta.set_stroke(color_borde, width=2 + progreso * 2)

            # Atmósfera: ROJA BRILLANTE al final (como sol)
            opacidad_atm = progreso * 0.6  # máximo 60% opacidad para brillo
            radio_atm = radio_planeta * (1.3 + progreso * 0.5)
            color_atm = interpolate_color(ORANGE, RED, progreso)  # naranja → rojo
            self.atmosfera.scale_to_fit_width(radio_atm * 2)
            self.atmosfera.set_fill(color_atm, opacity=opacidad_atm)
            self.atmosfera.set_stroke(RED, width=progreso * 4, opacity=progreso * 0.9)
...
# manim -pqh LeSage-v1.2.1.py LeSageComparacion
