...
class OceanoeCEL(Scene):
    """
    v2.1.2 - Efecto CASCADA de desplazamiento

    El eCEL desplazado por la masa TAMBIÉN desplaza el eCEL circundante.
    Todo se organiza en UN SOLO halo extendido con gradiente suave.

    ρ_total(r) = Σ ρ_nivel(r) → Halo más grande que v2.1.1
    """
    ...
    def construct(self):
        ...
        # 3. eCEL desplazado con efecto cascada (UN SOLO halo)
        texto_cascada = Text(
        ...
        # Crear halo completo con efecto cascada
        ecel_cascada = self.crear_ecel_cascada(
        ...
        texto_gradiente = Text(
            ...
            "Gradiente suave extendido con 1/r²",
            font_size=18,
            color=WHITE
        ).to_edge(DOWN)

        self.play(FadeOut(texto_cascada), Write(texto_gradiente))
        self.wait(2)

        # 5. Texto final
        texto_final = Text(
            f"Halo extendido: mayor alcance que desplazamiento simple\n"
    ...
    def crear_ecel_cascada(self, centro, densidad, radio_masa):
        """
        Crea UN SOLO halo con efecto cascada.

        El eCEL desplazado se acumula Y ese eCEL acumulado
        desplaza más eCEL del espacio circundante.

        Resultado: halo más extendido con gradiente suave.
        """
        ecel = VGroup()
        ...
        # Más capas que v2.1.1 para halo extendido
        num_capas = 20
        ...
        for capa in range(num_capas):
            ...
            # Densidad según 1/r² (efecto cascada incluido)
            # La densidad efectiva es mayor porque incluye el eCEL
            # desplazado por el eCEL acumulado en capas anteriores
            factor_r2 = (radio_masa / r) ** 2

            # Factor de cascada: las primeras capas "empujan" más
            # Esto crea un gradiente más suave y extendido
            factor_cascada = 1.0 + 0.3 * np.exp(-capa / 5)

            # Opacidad con efecto cascada
            opacidad_capa = opacidad_max * factor_r2 * factor * factor_cascada
            opacidad_capa = max(0.1, min(opacidad_capa, opacidad_max))

            # Partículas en este círculo
            circunferencia = 2 * np.pi * r
            particulas_capa = int(circunferencia / distancia_minima)
            particulas_capa = max(10, particulas_capa)

            # Color gradiente: más brillante cerca, más tenue lejos
            t = capa / num_capas
            color_capa = interpolate_color(BLUE_B, PURPLE_A, t)
            ...
            for i in range(particulas_capa):
                ...
                # Radio partícula decrece con distancia
